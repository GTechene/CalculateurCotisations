...
public class Calculateur2026 : ICalculateur
{
    ...
    public void CalculeLesCotisations(decimal revenu)
    {
        ...
        RetraiteDeBase = _calculateurCommun.CalculeLaRetraiteDeBase(revenu, Constantes2026.TauxCotisationsRetraiteBaseRevenusInferieursAuPass, Constantes2026.TauxCotisationsRetraiteBaseRevenusSuperieursAuPass, Constantes2026.RevenusPlancherRetraiteDeBase);
        RetraiteComplementaire = _calculateurCommun.CalculeLaRetraiteComplementaireSelonLeRegimeArtisansCommercants(revenu, Constantes2026.TauxRetraiteComplementairePremiereTrancheArtisansCommercants, Constantes2026.TauxRetraiteComplementaireDeuxiemeTrancheArtisansCommercants, PASS.Valeur);
    ...
    private ResultatAvecExplicationEtTaux CalculeLesCotisationsMaladieHorsIndemnites(decimal revenu)
    {
        ...
        {
            var tauxApplicable = CalculeLeTauxProgressif(revenu, PASS.Valeur * 0.2m, PASS.Valeur40Pct, 0m, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre20PctEt40PctDuPass);
            var cotisations = revenu * tauxApplicable;
            return new ResultatAvecTauxUniqueEtExplication(cotisations, $"L'assiette de {revenu:C0} est compris entre {PASS.Valeur * 0.2m:C0} (20% du PASS) et {PASS.Valeur40Pct:C0} (40% du PASS), donc un taux progressif entre 0% et {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre20PctEt40PctDuPass * 100:F1}% est appliqué. Ici il s'agit de {tauxApplicable * 100:F1}%, soit {cotisations:C0} de cotisations.", tauxApplicable);
        }

        if (revenu > PASS.Valeur40Pct && revenu <= PASS.Valeur60Pct)
        {
            var tauxApplicable = CalculeLeTauxProgressif(revenu, PASS.Valeur40Pct, PASS.Valeur60Pct, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre20PctEt40PctDuPass, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre40PctEt60PctDuPass);
            var valeur = revenu * tauxApplicable;
            return new ResultatAvecTauxUniqueEtExplication(valeur, $"L'assiette de {revenu:C0} est comprise entre {PASS.Valeur40Pct:C0} (40% du PASS) et {PASS.Valeur60Pct:C0} (60% du PASS), donc un taux progressif entre {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre20PctEt40PctDuPass * 100:F1}% et {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre40PctEt60PctDuPass * 100:N0}% est appliqué. Ici il s'agit de {tauxApplicable * 100:F1}%, soit {valeur:C0} de cotisations.", tauxApplicable);
        }

        if (revenu > PASS.Valeur60Pct && revenu <= PASS.Valeur110Pct)
        {
            var tauxApplicable = CalculeLeTauxProgressif(revenu, PASS.Valeur60Pct, PASS.Valeur110Pct, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre40PctEt60PctDuPass, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre60PctEt110PctDuPass);
            var valeur = revenu * tauxApplicable;
            return new ResultatAvecTauxUniqueEtExplication(valeur, $"L'assiette de {revenu:C0} est comprise entre {PASS.Valeur60Pct:C0} (60% du PASS) et {PASS.Valeur110Pct:C0} (110% du PASS), donc un taux progressif entre {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre40PctEt60PctDuPass * 100:N0}% et {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre60PctEt110PctDuPass * 100:F1}% est appliqué. Ici il s'agit de {tauxApplicable * 100:F1}%, soit {valeur:C0} de cotisations.", tauxApplicable);
        }

        if (revenu > PASS.Valeur110Pct && revenu <= PASS.Valeur * 2)
        {
            var tauxApplicable = CalculeLeTauxProgressif(revenu, PASS.Valeur110Pct, PASS.Valeur * 2, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre60PctEt110PctDuPass, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre110PctEt200PctDuPass);
            var valeur = revenu * tauxApplicable;
            return new ResultatAvecTauxUniqueEtExplication(valeur, $"L'assiette de {revenu:C0} est comprise entre {PASS.Valeur110Pct:C0} (110% du PASS) et {PASS.Valeur * 2:C0} (2 PASS), donc un taux progressif entre {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre60PctEt110PctDuPass * 100:F1}% et {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre110PctEt200PctDuPass * 100:F1}% est appliqué. Ici il s'agit de {tauxApplicable * 100:F1}%, soit {valeur:C0} de cotisations.", tauxApplicable);
        }

        if (revenu > PASS.Valeur * 2 && revenu <= PASS.Valeur * 3)
        {
            var tauxApplicable = CalculeLeTauxProgressif(revenu, PASS.Valeur * 2, PASS.Valeur * 3, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre110PctEt200PctDuPass, Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre200PctEt300PctDuPass);
            var valeur = revenu * tauxApplicable;
            return new ResultatAvecTauxUniqueEtExplication(valeur, $"L'assiette de {revenu:C0} est comprise entre {PASS.Valeur * 2:C0} (2 PASS) et {PASS.Valeur * 3:C0} (3 PASS), donc un taux progressif entre {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre110PctEt200PctDuPass * 100:F1}% et {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre200PctEt300PctDuPass * 100:F1}% est appliqué. Ici il s'agit de {tauxApplicable * 100:F1}%, soit {valeur:C0} de cotisations.", tauxApplicable);
        }
        ...
        if (revenu > PASS.Valeur * 3)
        {
            ...
            return new ResultatAvecExplicationEtTaux(valeur, $"L'assiette de {revenu:C0} est supérieure à {PASS.Valeur * 3:C0} (3 PASS), donc un taux de {Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre200PctEt300PctDuPass * 100:F1}% est appliqué à la part des revenus dans la limite de 3 PASS et le taux fixe de {Constantes2026.TauxPartReduiteCotisationsMaladiePourRevenusSupA300PctDuPass * 100:F1}% est appliqué à la part des revenus qui y est supérieure, soit {valeur:C0} de cotisations.", Constantes2026.TauxPlafondCotisationsMaladiePourRevenusEntre200PctEt300PctDuPass, Constantes2026.TauxPartReduiteCotisationsMaladiePourRevenusSupA300PctDuPass);
        }
